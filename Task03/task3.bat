...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "SELECT m.title, m.year FROM movies m JOIN ratings r ON m.id = r.movie_id GROUP BY m.id HAVING COUNT(r.rating) >= 1 ORDER BY m.year, m.title LIMIT 10;"
echo " "

echo "2. Вывести список всех пользователей, фамилии (не имена!) которых начинаются на букву 'A'. Полученный список отсортировать по дате регистрации. В списке оставить первых 5 пользователей."
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "SELECT id, name, register_date FROM users WHERE name LIKE '% A%' ORDER BY register_date LIMIT 5;"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "SELECT u.name AS expert_name, m.title AS movie_title, m.year AS movie_year, r.rating, date(r.timestamp, 'unixepoch') AS rating_date FROM ratings r JOIN users u ON r.user_id = u.id JOIN movies m ON r.movie_id = m.id ORDER BY u.name, m.title, r.rating LIMIT 50;"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "SELECT m.title, m.year, COUNT(*) as high_ratings_count FROM movies m JOIN ratings r ON m.id = r.movie_id JOIN users u ON r.user_id = u.id WHERE m.genres LIKE '%Drama%' AND m.year > 2005 AND r.rating >= 4.5 AND u.gender = 'F' GROUP BY m.id, m.title, m.year ORDER BY m.year, m.title;"
echo " "
...
echo --------------------------------------------------
echo "Количество пользователей по годам:"
sqlite3 movies_rating.db -box "SELECT SUBSTR(register_date, 1, 4) AS year, COUNT(*) AS user_count FROM users GROUP BY year ORDER BY year;"
echo "Год с наибольшим количеством регистраций:"
sqlite3 movies_rating.db -box "SELECT year, user_count FROM (SELECT SUBSTR(register_date, 1, 4) AS year, COUNT(*) AS user_count FROM users GROUP BY year) ORDER BY user_count DESC LIMIT 1;"
echo "Год с наименьшим количеством регистраций:"
sqlite3 movies_rating.db -box "SELECT year, user_count FROM (SELECT SUBSTR(register_date, 1, 4) AS year, COUNT(*) AS user_count FROM users GROUP BY year) ORDER BY user_count ASC LIMIT 1;"
